...
<!-- we get id of user which is login to the page -->
...
<!-- show the list of doctor in appointmtnt page -->
									<%
									DoctorDataAccess dda = new DoctorDataAccess(DBConnection.getConn());
									List<Doctor> list = dda.getAllDoctor();
									for (Doctor d : list) {
									%>
...
<!-- user is not login if user try to login then that data will not be saved and return back to user login page -->
							<c:if test="${empty userObj }">
								<a href="user_login.jsp" class="col-md-6 offset-md-3 btn btn-success">Submit</a>
							</c:if>

<!-- if user is login,then data will be stored -->
